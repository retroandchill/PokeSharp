        ...
        foreach (var item in source)
        {
            if (item is null)
            {
                if (!targetElementType.IsValueType || 
                    (targetElementType.IsGenericType && targetElementType.GetGenericTypeDefinition() == typeof(Nullable<>)))
                {
                    list.Add(null);
                }
                else
                {
                    throw new InvalidOperationException($"Cannot assign null to non-nullable type {targetElementType}");
                }
            }
