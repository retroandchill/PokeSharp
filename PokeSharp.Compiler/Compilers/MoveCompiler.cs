...
public class MoveCompiler(ILogger<MoveCompiler> logger) : PbsCompiler<Move, MoveInfo>
{
    ...
    protected override MoveInfo ValidateCompiledModel(MoveInfo model, FileLineData fileLineData)
    {
        ...
        if (model.Category == DamageCategory.Status || model.Power != 0) return model;
        
        logger.LogWarning("Move {Name} is defined as Physical or Special but has a base damage of 0. Changing it to a Status move.\n{Line}", model.Name, fileLineData.LineReport);
