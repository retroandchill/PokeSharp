...
public static class GameContextManager
{
    ...
    public static GameContext Current
    {
        get
        {
            if (ContextStack.Count > 0) return ContextStack.Peek();
            
            return _globalContext ?? throw new InvalidOperationException(
    ...
    {
        if (_globalContext is not null) throw new InvalidOperationException("GameContext already initialized.");
    ...
    {
        return ContextStack.Count > 0 ? ContextStack.Pop() : throw new InvalidOperationException("No scoped context available.");
    }
}