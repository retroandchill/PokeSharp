...
#define FIELD_ASSERTS()                                                                                                \
    ...
    using result_of_method_t =
        std::remove_pointer_t<std::remove_reference_t<std::invoke_result_t<FuncType, ObjectType *, Args...>>>;
  ...
  public:
    ...
    {
        return IsSet() ? TOptionalPtr<ReturnType>(
                             std::invoke(std::forward<FuncType>(func), m_obj, std::forward<Args>(args)...))
    ...
    {
        return IsSet() ? TOptionalPtr<ObjectType>(m_obj) : TOptionalPtr<ObjectType>(return_obj);
    }

    template <std::invocable<> Functor>
        requires std::assignable_from<ObjectType *&, std::invoke_result_t<Functor>>
    TOptionalPtr<ObjectType> OrElseGet(Functor &&Callback)
    {
        return IsSet() ? TOptionalPtr<ObjectType>(m_obj) : TOptionalPtr<ObjectType>(std::invoke(Callback));
    }

    /**
     * @brief Applies given member function to the wrapped object and returns the result if valid, default_value
     * otherwise
     * @tparam Args types of arguments provided to the member function (auto-deduced)
     * @tparam FuncType type of member function (auto-deduced)
     * @tparam ReturnType type of return object (auto-deduced)
    ...
    ReturnType MapToValue(const ReturnType &default_value, FuncType &&func, Args &&...args)
    {
        ...
        return IsSet() ? std::invoke(func, m_obj, std::forward<Args>(args)...) : default_value;
    }
    ...
    {
        return IsSet() ? std::invoke(field, m_obj) : default_value;
    }
    ...
    {
        return IsSet() ? m_obj : return_value;
    }
