...
namespace PokeEdit {
    ...
    {
        using FMembers = TMemberVariant<decltype(TJsonObjectContainer<T>::JsonSchema)>;
        TMap<FName, FMembers> Result;
        TJsonObjectContainer<T>::JsonSchema.ForEachField([&]<auto Member>(const TJsonField<Member>& Field) 
            {
    ...
    std::expected<void, FString> ApplyEdit(T &Value, const FDiffNode& DiffNode)
    {
        if (auto *Set = DiffNode.TryGet<FValueSetNode>(); Set != nullptr)
        {
            return DeserializeFromJson<T>(Set->NewValue)
                .transform([&Value](T &&ParsedValue)
                {
                    Value = MoveTemp(ParsedValue);
                });
        }
   ...
   std::expected<void, FString> ApplyEdit(T &Value, const FObjectDiffNode &ObjectDiff)
    {
        ...
        for (const auto &[FieldName, Field] : ObjectDiff.Properties)
        {
            ...
            {
                return std::unexpected(FString::Format(TEXT("Field '{0}' does not exist in object"), {FieldName.ToString()}));
            }
                
            return Visit([&]<auto Member>(const TJsonField<Member>& JsonField) -> std::expected<void, FString>
                { 
        ...
        return DiffNode.Visit(UE::Overload(
            [&Value](const FValueSetNode& Node)
            {
                return DeserializeFromJson<T>(Node.NewValue)
                    .transform([&Value](T &&DeserializedValue)
                    {
                        Value.Emplace(MoveTemp(DeserializedValue));
                    });
            },
            [&Value](const FListDiffNode& ListDiffNode)
            {
                return ApplyEdit(Value, ListDiffNode);
            },
        ...
        return DiffNode.Visit(UE::Overload(
            [&Value](const FValueSetNode& SetNode) -> std::expected<void, FString>
            {
                return DeserializeFromJson<TMap<K, V>>(SetNode.NewValue)
                    .transform([&Value](TMap<K, V> &&DeserializedValue)
                    {
                        Value = MoveTemp(DeserializedValue);
                    });
            },
            [&Value](const FDictionaryDiffNode& DictionaryDiffNode)
            {
                return ApplyEdit(Value, DictionaryDiffNode);
            },
            [](auto &&) -> std::expected<void, FString>
            {
                return std::unexpected(TEXT("Invalid diff node type"));
            ...
            auto Result = Edit.Visit(UE::Overload(
                [&Value](const FDictionarySetNode& SetNode) -> std::expected<void, FString>
                {
                    return DeserializeFromJson<K>(SetNode.Key)
                        .and_then([&SetNode, &Value](const K &DeserializedKey) -> std::expected<void, FString>
                        {
                ...
                [&Value](const FDictionaryAddNode& AddNode) -> std::expected<void, FString>
                {
                    return DeserializeFromJson<K>(AddNode.Key)
                        .and_then([&Value, &AddNode](K &DeserializedKey) -> std::expected<void, FString>
                        {
                            ...
                            return DeserializeFromJson<V>(AddNode.Value)
                                .transform([&Value, &DeserializedKey](V &&DeserializedValue)
                                {
                ...
                [&Value](const FDictionaryRemoveNode& RemoveNode) -> std::expected<void, FString>
                {
                    return DeserializeFromJson<K>(RemoveNode.Key)
                        .and_then([&Value](const K &DeserializedKey) -> std::expected<void, FString>
                        {
                            if (!Value.Contains(DeserializedKey))
                            {
                ...
                [&Value](const FDictionaryChangeKeyNode& ChangeKeyNode) -> std::expected<void, FString>
                {
                    return DeserializeFromJson<K>(ChangeKeyNode.OldKey)
                        .and_then([&Value, &ChangeKeyNode](const K &OldKey) -> std::expected<void, FString>
                        {
                            ...
                            return DeserializeFromJson<K>(ChangeKeyNode.NewKey)
                                .transform([&Value, &OldKey](const K &NewKey)
                                {
    ...
    TOptional<FDiffNode> Diff(const T& OldValue, const T& NewValue)
    {
        if constexpr (TValidJsonObjectContainer<T>)
        {
            ...
            TJsonObjectContainer<T>::JsonSchema.ForeEachField([&]<auto Member>(const TJsonField<Member>& Field)
            {
        ...
        {
            return OldValue != NewValue ? TOptional<FDiffNode>(InPlace, TInPlaceType<FValueSetNode>{}, SerializeToJson(NewValue)) : TOptional<FDiffNode>();
        }
    }
    ...
    TOptional<FDiffNode> Diff(const TOptional<T>& OldValue, const TOptional<T>& NewValue)
    {
        if (!OldValue.IsSet())
        {
            return NewValue.IsSet() ? TOptional<FDiffNode>(InPlace, TInPlaceType<FValueSetNode>{}, SerializeToJson(NewValue)) : TOptional<FDiffNode>();
        }
